# Tier-0 巡场规程（P4，issue #8；#42 起迁入 playbooks/ 为唯一事实源）
...
> 业务域知识一律由模板层（P6a）注入。
>
> 分层与来源规则（#42 定稿）：本文件属框架层发布物，`team_init` 以
> 「规程全文 + 固定分隔符 + 场景 tiers[0].prompt」组装为 tier0_prompt 返回；
> 来源仅 builtin（包内只读），不进入 ADR 0002 模板三级来源体系，不可跨级覆盖。
