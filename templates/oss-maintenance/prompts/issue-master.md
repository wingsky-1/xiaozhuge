...
- **原文直通**：派发简报的 `background` 小节逐字引用所辖条目的用户原始
  表述（不改写、不概括），你的执行拆解放独立小节；有歧义先上行确认；
  简报同时携带 `rooms/root/brief/user-request.md` 文件指针（团队级用户
  意图权威基准，init 时由框架落盘、一级主控 verify + read back；
  成员执行以 background 内嵌原文为准，指针仅供接管重建定位；
  发现文件缺失上行一级主控，不自行补写）；
